...
O diretório de RPMS (OU DEBS) também contém um pacote com o nome libreoffice7.4-freedesktop-menus-7.4.0.1-1.noarch.rpm (ou libreoffice7.4-debian-menus_7.4.0.1-1_all.deb, respetivamente, ou similar). Este é o pacote a utilizar para todas as distribuições Linux que cumpram as especificações/recomendações Freedesktop.org (https://en.wikipedia.org/wiki/Freedesktop.org) e é disponibilizado para a instalação em outras distribuições Linux que não estejam abrangidas pelas instruções acima mencionadas.
...
Esperamos que o novo LibreOffice 7.4 corresponda às suas expectativas e aguardamos a sua integração na equipa.
